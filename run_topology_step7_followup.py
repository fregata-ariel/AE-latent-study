"""Run the Step 7 topology Phase A/B follow-up with a full anchor rerun."""
...
def _load_step7_summaries(path: str) -> dict[str, dict]:
    with open(path) as f:
        return json.load(f)


def _select_step7_winner(summary_path: str) -> tuple[str, dict, bool]:
    summaries = _load_step7_summaries(summary_path)
    winner_name, winner_summary, used_gate = _select_best_run(summaries)
    if winner_name is None or winner_summary is None:
        raise RuntimeError('No Step 7 summaries found; run Step 7 experiments first.')
...
def run_all(
    base_dir: str = 'runs',
    step7_summary_path: str = 'runs/lattice_step7_summaries.json',
    diagnostics_dir: str = 'runs/topology_diagnostics_step7',
    phasea_report_path: str = 'walkthrough-topology-step7-phaseA.md',
    phaseb_report_path: str = 'walkthrough-topology-step7-phaseB.md',
    roadmap_path: str = 'ae-latent-study-roadmap-step7.md',
    config_overrides: dict | None = None,
) -> dict:
    """Run topology Phase A/B follow-up for the selected Step 7 winner."""
    os.makedirs(diagnostics_dir, exist_ok=True)
    config_overrides = config_overrides or DEFAULT_TOPOLOGY_OVERRIDES
    winner_name, winner_summary, used_gate = _select_step7_winner(step7_summary_path)
    ...
    with open(os.path.join(diagnostics_dir, 'step7_followup_summary.json'), 'w') as f:
        json.dump(combined, f, indent=2)

    print(f"\nSelected Step 7 winner: {winner_name}")
